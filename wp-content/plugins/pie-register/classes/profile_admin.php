...
class Profile_admin extends PieReg_Base
{
    ...
    function addDropdown()
    {
		...
		echo '<select ' . $multiple . ' id="' . $this->id . '" name="' . $this->slug . '" class="' . $this->field['css'] . '" style="min-width:'.$width.'px;" >';
			...
			else if($this->field['list_type']=="months")
			{
				...
				for($a = 1; $a<=sizeof($mons);$a++){
					$sel = 'selected="selected"';
					echo '<option value = "'.$a.'">'.__($mons[$a],"piereg").'</option>';
				}
			}
			...
			//print_r($this->field['value']);
				if (sizeof($this->field['value']) > 0) {
					for ($a = 0; $a < sizeof($this->field['value']); $a++) {
						$selected = '';
						if(isset($sel_options) && is_array($sel_options)){
