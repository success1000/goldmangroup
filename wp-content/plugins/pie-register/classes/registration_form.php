...
class Registration_form extends PieReg_Base
{
	...
	function addFormData()
	{
		...
		$data = "";
		$data .= '<div class="tabs">';
		$data .= '<ul class="tab-links">';
		$data	.= '<li class="active"><a href="#tab1">Account Information</a></li>';
    $data .= '<li><a href="#tab2">Company Profile</a></li>';
		$data .= '<li><a href="#tab3">Extended Profile</a></li>';       
 		$data .= '</ul>';
	...
	function addDefaultField()
	{
		...
		{
			$data .= '<div><input id="'.$this->id.'" name="'.$this->field['field_name'].'" class="'.$this->addClass().'"  placeholder="'.(isset($this->field['placeholder'])?$this->field['placeholder']:"").'" type="text" value="'.$this->getDefaultValue().'" /></div>';	
		}	
		return $data;
	}
	function addTextField()
	{
		return '<div><input id="'.$this->id.'" name="'.$this->name.'" class="'.$this->addClass().'"  '.$this->addValidation().'  placeholder="'.$this->field['placeholder'].'" type="text" value="'.$this->getDefaultValue().'" /></div>';
	}
	function addHiddenField()
	{
		return '<div id="tab2" class="tab">';		
	}
	function addUsername($form_widget = false)
	{
		$formwidget = (isset($form_widget) && $form_widget == true)? '_widget' : '';
		return '<div><input id="username'.$formwidget.'" name="username" class="input_fields piereg_validate[required,username]" placeholder="'.$this->field['placeholder'].'" type="text" value="'.$this->getDefaultValue('username').'" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'"  /></div>';	
	...
	function addPassword($fromwidget)
	{
		...
		$data .= 'id="'.$this->id.'" name="password" class="'.$this->addClass("input_fields",array("minSize[8]")).'" placeholder="'.$this->field['placeholder'].'" type="password" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-underflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-overflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" autocomplete="off" /></div>';
		...
		$data .= '</div></li><li class="fields pageFields_'.$this->pages.' '.$topclass.'"><div class="fieldset"><label>'.$label2.'</label><div><input id="confirm_password_'.$this->id.'" name="confirm_password" type="password" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-underflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-overflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" class="input_fields '.$this->field['css'].' piereg_validate[required,equals['.$this->id.']]" placeholder="'.$this->field['placeholder'].'" autocomplete="off" /></div>';	
	...
	function addEmail($fromwidget)
	{
		...
		$data .='<div><input id="'.$this->id.'" name="e_mail" class="'.$this->addClass().'"  '.$this->addValidation().'  placeholder="'.$this->field['placeholder'].'" type="text" value="'.$this->getDefaultValue("e_mail").'" /></div>';
	...
	function addUpload()
	{
		return '<div><input id="'.$this->id.'" name="'.$this->name.'" class="'.$this->addClass().'"  '.$this->addValidation().' type="file" /></div>';
	}
	function addProfilePicUpload()
	{
		return '<div><input id="'.$this->id.'" name="'.$this->name.'" class="'.$this->addClass("input_fields",array("funcCall[checkExtensions],ext[gif|GIF|jpeg|JPEG|jpg|JPG|png|PNG|bmp|BMP]")).'"  '.$this->addValidation().' type="file"  /></div>';	
	}
	...
	function addDropdown()
	{
		...
		$data .= '<div><select '.$multiple.' id="'.$name.'" name="'.$name.'" class="'.$this->addClass("").'" '.$this->addValidation().'  >';
	...
	{
return '<div id="tab3" class="tab">';		
	}
	function addPhone()
	{		
		return '<div><input id="'.$this->id.'" class="'.$this->addClass().'"  '.$this->addValidation().' name="'.$this->name.'"  placeholder="'.((isset($field['placeholder']))?$field['placeholder']:"").'" type="text" value="'.$this->getDefaultValue().'" /></div>';	
	}
	function addList()
	{
		$data = "";
...
//		$data = '<div class="piereg-html-field-content" >';
	...
	function printFields($fromwidget = false)
	{
        ...
        if ($this->field['type'] == "sectionbreak")
				{
