...
class Edit_form extends PieReg_Base
{
	...
	function addFormData()
	{
		...
		return '<h2 id="piereg_pie_form_heading">'.__("Complete your profile","piereg").'</h1>';
	}
...
//		return '<input id="'.$this->id.'" name="'.$this->name.'"  type="hidden" value="'.$val.'" />';		
    return '<div style="max-width:460px;float:left">';
	}
...
//			$topclass = "";
	...
	{
	return '<div style="float:left;max-width:460px;margin-left:20px">';
  }
	function addPhone()
	{		
		//$val = get_usermeta($this->user->data->ID , $this->slug);
	...
	{
		return '<div style="margin-right:30px;float:left;max-width:460px">';	
	}
	...
	function editProfile($user){
        ...
        if ($this->field['type'] == "username")
			{
				$profile_fields_data .= '<div style="display:none">'.$this-> addUsername().'</div>';
				continue;
			}
      
       if ($this->field['type'] == "email")
			{
				$profile_fields_data .= '<div style="display:none">'.$this-> addEmail().'</div>';
				continue;
			}
      
        if ($this->field['type'] == "password")
			{
				$profile_fields_data .= '<div style="display:none">'.$this-> addPassword().'</div>';
				continue;
			}
	...
	if ($this->field['type'] == "sectionbreak")
      	...
      	if ($this->field['type'] == "number")
			...
			$profile_fields_data .= '<li class="fields pageFields_'.$this->pages.' '.$topclass.'">';
     // $profiles_fields_data .= '<li class="fields pageFields_'.$this->pages.' '.$topclass.'">';
			...
			//	case 'number':
				...
				case 'list':							
				case 'default':
					$profile_fields_data .= '<div class="fieldset"><div style="margin-bottom:5px;font-weight:bold">'.$this->addLabel().'</div>';
				break;
  //   	case 'password':
	...
	//			break;
			...
			switch($this->field['type']) :
				...
				case 'text' :								
				case 'website' :
				$profile_fields_data .= '<div>'.$this->addTextField().'</div>';
				break;				
				case 'username' :
				$profile_fields_data .= $this->addUsername();
				break;
				...
				case 'dropdown':
				case 'multiselect':
				$profile_fields_data .= '<div>'.$this->addDropdown().'</div>';
				break;
			//	case 'number':
			//	$profile_fields_data .= $this->addNumberField();			
			//	break;
				case 'radio':
				case 'checkbox':
				$profile_fields_data .= '<div>'.$this->addCheckRadio().'</div>';
				break;
				case 'name':
				$profile_fields_data .= $this->addName();
				break;
				...
				break;
				case 'upload':
				$profile_fields_data .= '<div>'.$this->addUpload().'</div>';
				break;
				case 'profile_pic':
				$profile_fields_data .= '<div>'.$this->addProfilePic().'</div>';
				break;
				case 'address':
				$profile_fields_data .= '<div>'.$this->addAddress().'</div>';
				break;				
				case 'phone':
				$profile_fields_data .= '<div>'.$this->addPhone().'</div>';
				break;
				case 'date':
				$profile_fields_data .= $this->addDate();			
				break;
